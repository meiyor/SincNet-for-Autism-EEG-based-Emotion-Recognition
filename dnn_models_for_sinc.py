        ...
        for i in range(self.N_cnn_lay):
            ...
            if self.cnn_use_laynorm[i]:
                if i == 0:
                    ...
                    x = self.drop[i](self.act[i](self.ln[i](F.max_pool2d(self.conv[i](
                        x), [self.cnn_max_pool_len[i][0], self.cnn_max_pool_len[i][1]]))))

            if self.cnn_use_batchnorm[i]:
                x = self.drop[i](self.act[i](self.bn[i](F.max_pool2d(self.conv[i](
                    x), [self.cnn_max_pool_len[i][0], self.cnn_max_pool_len[i][1]]))))

            if self.cnn_use_batchnorm[i] is False and self.cnn_use_laynorm[i] is False:
                x = self.drop[i](self.act[i](F.max_pool2d(self.conv[i](
                    x), [self.cnn_max_pool_len[i][0], self.cnn_max_pool_len[i][1]])))
