...
class SincConv_fast(nn.Module):
    ...
    def __init__(self, out_channels, kernel_size, sample_rate=500, in_channels=1,
...
class sinc_conv(nn.Module):
    ...
    def __init__(self, N_filt,Filt_dim,fs):
        ...
        low_freq_mel = 0.1
        high_freq_mel = 30 # Convert Hz to Mel based on the limits of the EEG frequency range
    ...
    def __init__(self,options):
       ...
       for i in range(self.N_cnn_lay):
         ...
         if i==0:
          ...
          self.conv.append(nn.Conv2d(self.cnn_N_filt[i-1], self.cnn_N_filt[i], [self.cnn_len_filt[i][0],self.cnn_len_filt[i][1]]))
       ...
       for i in range(self.N_cnn_lay):
         ...
         if self.cnn_use_laynorm[i]:
          if i==0:
           ...
           x = self.drop[i](self.act[i](self.ln[i](F.max_pool2d(self.conv[i](x),[self.cnn_max_pool_len[i][0],self.cnn_max_pool_len[i][1]]))))   
          
         if self.cnn_use_batchnorm[i]:
          x = self.drop[i](self.act[i](self.bn[i](F.max_pool2d(self.conv[i](x),[self.cnn_max_pool_len[i][0],self.cnn_max_pool_len[i][1]]))))

         if self.cnn_use_batchnorm[i]==False and self.cnn_use_laynorm[i]==False:
          x = self.drop[i](self.act[i](F.max_pool2d(self.conv[i](x),[self.cnn_max_pool_len[i][0],self.cnn_max_pool_len[i][1]])))
