...
from data_io_new import read_conf, str_to_bool
from dnn_models_for_sinc import MLP
...
except output_folder.DoesNotExist:
...
np.savetxt('frequency_ranges_'+strn1[7] +
           '_'+sys.argv[2]+'.txt', CNN_net.conv[0].hz)
...
for epoch in range(N_epochs):
    ...
    for i in range(N_batches):
        ...
        [input, label] = create_batches_rnd(
            batch_size, data_folder, wlen, 0.2, int(sys.argv[2])-1, epoch)
        pout = DNN2_net(DNN1_net(CNN_net(input)))
        pred = torch.max(pout, dim=1)[1]
        loss = cost(pout, label.long())
        err = torch.mean((pred != label.long()).float())

        err_happy = np.mean((label.data).cpu().numpy()[np.where((label.data).cpu().numpy() == 0.0)[
                            0]] != (pred.data).cpu().numpy()[np.where((label.data).cpu().numpy() == 0.0)[0]])
        err_sad = np.mean((label.data).cpu().numpy()[np.where((label.data).cpu().numpy() == 1.0)[
                          0]] != (pred.data).cpu().numpy()[np.where((label.data).cpu().numpy() == 1.0)[0]])
        err_angry = np.mean((label.data).cpu().numpy()[np.where((label.data).cpu().numpy() == 2.0)[
                            0]] != (pred.data).cpu().numpy()[np.where((label.data).cpu().numpy() == 2.0)[0]])
        err_fear = np.mean((label.data).cpu().numpy()[np.where((label.data).cpu().numpy() == 3.0)[
                           0]] != (pred.data).cpu().numpy()[np.where((label.data).cpu().numpy() == 3.0)[0]])
...
# Full Validation  new
    ...
    if epoch % N_eval_epoch == 0:
        ...
        with torch.no_grad():
            for p in range(1):
                ...
                data_test = data_test.reshape(
                    num_rows_t, num_cols_t-1)  # 2D Matrix of data points
                # check if thre is some incompatibility
                ...
                signal = torch.from_numpy(
                    data_test_t).float().cuda().contiguous()
            ...
            np.savetxt('filters_vals_'+str(epoch)+'_'+strn1[7]+'_'+sys.argv[2]+'.txt', np.squeeze(
                CNN_net.conv[0].filters.cpu().numpy()), delimiter=',')
            with open('folder_output'+'/res_error_'+strn1[7]+'_'+sys.argv[2]+'.csv', 'a') as error_file:
                for err_dat in err_val:
        ...
        print("train-epoch %i, error_happy: %f, error_sad: %f, error_angry: %f, error_fear: %f" %
              (epoch, err_toth, err_tots, err_tota, err_totf))
